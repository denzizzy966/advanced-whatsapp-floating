...
// Pastikan variabel settings terdefinisi dengan default values
...
<div id="awf-floating-container" class="awf-floating-container <?php echo esc_attr($position_class); ?>" style="
...
">
    <!-- Floating Button -->
    <div id="awf-floating-button" class="awf-floating-button <?php echo esc_attr($animated_class); ?>">
    ...
    <!-- Contact Form Modal -->
    <div id="awf-contact-form" class="awf-contact-form <?php echo esc_attr($form_size_class); ?>" style="display: none;">
...
</div>-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893A11.821 11.821 0 0020.465 3.516"/>
                        </svg>
                    </button>
                </div>
            </form>
...
(function() {
    ...
    function initAWF() {
        ...
        var closeBtn = form ? form.querySelector('.awf-close-button') : null;
        ...
        if (contactForm) {
            contactForm.addEventListener('submit', function(e) {
                ...
                // Check if awf_ajax is available
                if (typeof awf_ajax !== 'undefined' && awf_ajax.nonce) {
                    formData.append('nonce', awf_ajax.nonce);
                }
                
                // Submit via XMLHttpRequest
                var xhr = new XMLHttpRequest();
                xhr.open('POST', typeof awf_ajax !== 'undefined' ? awf_ajax.ajax_url : '<?php echo admin_url('admin-ajax.php'); ?>');
                xhr.onreadystatechange = function() {
                    if (xhr.readyState === 4) {
                        ...
                        try {
                            ...
                            if (response.success) {
                                var successMsg = response.data && response.data.message ? response.data.message : 'Message sent successfully!';
                                ...
                                var actionAfterSubmit = typeof awf_ajax !== 'undefined' ? awf_ajax.action_after_submit : 'close_form';
                                if (actionAfterSubmit === 'close_form') {
                                    setTimeout(hideForm, 2000);
                                } else if (actionAfterSubmit === 'reset_form') {
                                    contactForm.reset();
                                }
                            } else {
